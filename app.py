...
def result_main():
    ...
    # --- タイプ別説明文 ---
    descriptions = {
        "貯蓄優先型": "リスクを抑えて貯蓄を優先するタイプ。安定的に資産を増やしたい人向けです。",
        "積立安定型（インデックスメイン）": "インデックス投資を中心に、長期で資産をコツコツ増やすタイプです。",
        "アクティブチャレンジ型（短期トレード型）": "短期トレードで高リターンを狙うタイプ。積極的な取引が特徴です。",
        "ステーキング運用型（仮想通貨積立・ステーキング）": "仮想通貨の積立＋ステーキングでコツコツ運用するタイプです。",
        "株式アクティブ型（中期〜長期の個別株投資）": "中期〜長期の個別株に投資して、リスクとリターンのバランスを狙います。",
        "ハイリスクハイリターン型（仮想通貨トレード型）": "リスクを覚悟して仮想通貨で大きなリターンを狙うタイプです。",
        "テクノロジー志向型（仮想通貨プロジェクト投資・新興市場）": "新興市場やテクノロジー関連のプロジェクトに投資するタイプです。",
        "積立型（50代以上特別判定）": "50代以上向けの長期積立型。リスクを抑えつつ安定運用を目指します。",
        "積立応用型（高配当株など）": "高配当株などで積立を応用的に運用するタイプです。"
    }
